...
//       "crop_height and crop_width to be set at the same time.";
  ...
  vector<int> top_shape_img = this->data_transformer_->InferBlobShape(cv_img);
  this->transformed_data_.Reshape(top_shape_img);
  
  // Reshape prefetch_data and top[0] according to the batch_size.
  const int batch_size = this->layer_param_.image_data_param().batch_size();
  CHECK_GT(batch_size, 0) << "Positive batch size required";
  top_shape_img[0] = batch_size;
  for (int i = 0; i < this->prefetch_.size(); ++i) {
    this->prefetch_[i]->data_.Reshape(top_shape_img);
  }
  top[0]->Reshape(top_shape_img);
  LOG(INFO) << "output data size: " << top[0]->num() << ","
      << top[0]->channels() << "," << top[0]->height() << ","
      << top[0]->width();
      
  //label
  vector<int> top_shape_lab = this->data_transformer_->InferBlobShape(cv_lab);
  this->transformed_label_.Reshape(top_shape_lab);
  top_shape_img[0] = batch_size;
  for (int i = 0; i < this->prefetch_.size(); ++i) {
    this->prefetch_[i]->label_.Reshape(top_shape_lab);
  }
  top[1]->Reshape(top_shape_lab);
...
//   const int crop_size = this->layer_param_.transform_param().crop_size();
  ...
  cv::Mat cv_lab = ReadImageToCVMat(root_folder + lines_[lines_id_].second,
                                    new_height, new_width, false, true);
  CHECK(cv_lab.data) << "Could not load " << lines_[lines_id_].first;
  ...
  top_shape_img[0] = batch_size;
  top_shape_lab[0] = batch_size;
  batch->data_.Reshape(top_shape_img);
  batch->label_.Reshape(top_shape_lab);
